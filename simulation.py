"""
simulation.py — EPW parsing + OCHRE driver.

Merged from:
    simulation/epw_parser.py
    simulation/ochre_runner.py
"""
...
# ===== from simulation/ochre_runner.py =====

# ---------------------------------------------------------------------------
# Thermal engine
# ---------------------------------------------------------------------------
# The 2R1C surrogate in run_single_simulation() is the engine used for every
# result in the report, whether or not OCHRE happens to be installed. The flag
# below is recorded for provenance only; nothing branches on it.
OCHRE_AVAILABLE = False
try:
    from ochre import Dwelling  # type: ignore  # noqa: F401
    OCHRE_AVAILABLE = True
except ImportError:
    pass
logger.info("Thermal engine: 2R1C surrogate (OCHRE installed: %s).", OCHRE_AVAILABLE)
