...
# ---------------------------------------------------------------------------
for d in [DATA_RAW, DATA_PROCESSED, TABLES_DIR, FIGURES_DIR]:
...
def run_tests() -> bool:
    ...
    result = subprocess.run(
        [sys.executable, "-m", "pytest", str(PROJECT_ROOT / "tests.py"), "-v", "--tb=short"],
        capture_output=True,
        text=True,
...
def main() -> None:
    ...
    from simulation import generate_weather, load_weather_profiles, run_all_simulations
    ...
    # ---- 3. HDD benchmark ----
    from pipeline import run_hdd_benchmark

    logger.info("=== Step 3: HDD benchmark ===")
    errors_df, hdd_model = run_hdd_benchmark(sim_df, cfg, TABLES_DIR)

    # ---- 4. ANOVA decomposition ----
    from pipeline import run_anova

    logger.info("=== Step 4: ANOVA decomposition ===")
    anova_table = run_anova(sim_df, TABLES_DIR)

    # ---- 5. Interaction regression ----
    from pipeline import run_interaction_regression

    logger.info("=== Step 5: Interaction regression ===")
    regression_results = run_interaction_regression(sim_df, cfg, TABLES_DIR)

    # ---- 6. Demand surfaces ----
    from pipeline import build_demand_surfaces
    ...
    master_df = write_master_results(sim_df, errors_df, TABLES_DIR / "master_results.csv")
